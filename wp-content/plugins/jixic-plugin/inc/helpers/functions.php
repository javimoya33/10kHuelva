...
function jixic_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('jixic_redux_custom_fonts_load', 'jixic_redux_custom_fonts_load');
function jixic_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = jixic_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/jixic/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [jixic_social_share_output description]
 *
 ...
 */
function jixic_social_share_output( $icon, $color = false ) {
...
add_action('jixic_social_share_output', 'jixic_social_share_output');


function jixic_like_us($PostID = '')
{ 
$like = get_post_meta( $PostID, '_jolly_like_it', true );
?>
<li><a href="javascript:void(0);" class="jolly_like_it indus-btn" data-id="<?php the_ID(); ?>"><span class="icon-heart"></span> <?php echo (int)$like;?></a></li>
<?php } ?>
