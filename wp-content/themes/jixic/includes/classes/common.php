...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				...
				'layout'            => jixic_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),

				'sidebar_type'      => jixic_meta( 'sidebar_source_type', $page_id, 'd' ),

				'sidebar_elementor' => jixic_meta( 'sidebar_elementor_template', $page_id ),

				'sidebar'           => jixic_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),

				'banner_type'       => jixic_meta( 'banner_source_type', $page_id ),

				'banner_elementor'  => jixic_meta( 'banner_elementor_template', $page_id ),

				'banner'            => jixic_set( jixic_meta( 'banner_page_background', $page_id ), 'url' ),

				'title'             => jixic_meta( 'banner_banner_title', $page_id ),
				
				'text'             => jixic_meta( 'banner_banner_text', $page_id ),

				'enable_banner'     => jixic_meta( 'banner_page_banner', $page_id ),
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : jixic_the_title( $template ),
				
				'text'         => $options->get( $text ) ? $options->get( $text ) : jixic_the_title( $template ),

				'banner'        => jixic_set( $bg, 'url' ),
	...
	public function single( $template = 'single' ) {
		...
		$banner_type = jixic_meta( 'style_two', $page_id );
		...
		$return = [
			...
			'banner_type'       => ($banner_type) ? $banner_type : 'one',
